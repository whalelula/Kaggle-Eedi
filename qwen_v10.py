...
VER = "qwen_v10"
NUM_PROC = 8
DATA_PATH_TRAIN = "../misconception_data_all_train_gpt-4o.csv"
DATA_PATH_VAL = "../val_misconceptions.csv"
DATA_PATH_MAP = "../misconception_mapping.csv"
...
class CustomSimCSEModel(nn.Module):
    ...
    def forward(self, query, doc):
        ...
        scores2 = scores2.view(query_emb.size(0), -1)
...
class Config:
    ...
    freeze_layers: int = 0
    lr: float = 3e-5
    warmup_steps: int = 3 if debug else 0
...
training_args = TrainingArguments(
    ...
    metric_for_best_model="map_at_25",
...
lora_config = LoraConfig(
    ...
    #layers_to_transform=[i for i in range(46) if i >= config.freeze_layers],
...
val_misconid_df = pd.read_csv(DATA_PATH_VAL).head(8)
...
# %%
train_df = new_train_df#new_train_df[~new_train_df['answer_id'].isin(val_misconceptions)].reset_index(drop=True)
